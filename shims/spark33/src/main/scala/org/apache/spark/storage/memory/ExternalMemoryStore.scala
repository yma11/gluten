...
import org.apache.spark.internal.config.{EXTERNAL_MEMORY_STORAGE_FRACTION, MEMORY_OFFHEAP_SIZE, MEMORY_STORAGE_FRACTION}
...
//    NativeBackendInitializer.setAsyncDataCache(size, reservationListener)
    if (GlutenConfig.getConf.enableVeloxCache) {
      // scalastyle:off println
      println("*******Will initializing data cache late in velox backend")
      // scalastyle:on println
      val size = (conf.get(MEMORY_OFFHEAP_SIZE) * conf.get(MEMORY_STORAGE_FRACTION)
        * conf.get(EXTERNAL_MEMORY_STORAGE_FRACTION)).toLong
